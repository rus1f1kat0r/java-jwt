...
class ECDSAAlgorithm extends Algorithm {
    ...
    public void verify(DecodedJWT jwt) throws SignatureVerificationException {
        ...
        try {
            ...
            boolean valid =
                    crypto.verifySignatureFor(getDescription(), publicKey, jwt.getHeader(), jwt.getPayload(), JOSEToDER(signatureBytes));
    ...
    byte[] DERToJOSE(byte[] derSignature) throws SignatureException {
        ...
        //Retrieve R number
        System.arraycopy(derSignature, offset + Math.max(-sPadding, 0), joseSignature, ecNumberSize + Math.max(sPadding, 0),
